...
def main():
    yield_df = pd.read_csv("data/raw/cereal_yield_worldbank.csv")
    ...
    # Lag feature: this country's own yield the prior year. Real farmers/
    # analysts have this at prediction time (it's last year's harvest, not
    # a leak of the current year), same justification as stress_lag1 in
    # crop-stress-prediction.
    df["yield_lag1"] = df.groupby("country_iso3")["cereal_yield_kg_ha"].shift(1)
